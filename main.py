...
# ====================================
# Состояния для записи пользователей
...
# ====================================
TIMEZONES = {
    'utc3': 'UTC+3 (Москва)', 'utc4': 'UTC+4 (Самара)',
    'utc5': 'UTC+5 (Екатеринбург)', 'utc7': 'UTC+7 (Красноярск/Новосибирск)',
    'utc10': 'UTC+10 (Владивосток)', 'custom': 'Другой часовой пояс'
}
...
def is_slot_blocked(date, time_slot):
    """Проверка блокировки временного слота"""
...
def get_days_keyboard(offset=0):
    ...
    nav = []
    if offset > 0: nav.append(InlineKeyboardButton("⬅️ Раньше", callback_data=f'dates_prev_{offset}'))
    if offset + 7 <= 14: nav.append(InlineKeyboardButton("Позже ➡️", callback_data=f'dates_next_{offset}'))
...
def get_time_keyboard(date):
    ...
    for i in range(0, len(slots), 2):
        ...
        if i + 1 < len(slots): row.append(InlineKeyboardButton(slots[i + 1], callback_data=f'time_{slots[i + 1]}'))
        keyboard.append(row)
...
# ====================================
# ОБРАБОТЧИКИ - ОСНОВНОЕ МЕНЮ
# ====================================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /start"""
    user = update.effective_user
    log_user_action(user, "Запустил /start")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'trial':
        ...
        log_user_action(query.from_user, "Начал запись")
...
async def level_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_user_action(query.from_user, f"Выбрал уровень: {level}")
...
async def instrument_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'inst_none':
        log_user_action(user, "Нет инструмента")
    ...
    log_user_action(user, f"Инструмент: {inst}")
...
async def timezone_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(f"✅ Часовой пояс: **{TIMEZONES[tz_key]}**\n\n📅 **Выберите день:**", 
                                   parse_mode='Markdown', reply_markup=get_days_keyboard(0))
    return DAY

async def custom_timezone_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка пользовательского часового пояса"""
    try:
...
async def day_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('dates_prev_') or query.data.startswith('dates_next_'):
        ...
        await query.edit_message_text(f"✅ Часовой пояс: **{context.user_data['timezone']}**\n\n📅 **Выберите день:**",
                                     parse_mode='Markdown', reply_markup=get_days_keyboard(new_offset))
        return DAY
    ...
    await query.message.reply_text(f"✅ День: **{format_date(selected_date)}**\n\n🕐 **Выберите время:**",
                                   parse_mode='Markdown', reply_markup=get_time_keyboard(selected_date))
    return TIME

async def time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Финальный выбор времени"""
    query = update.callback_query
    await query.answer()
    user = query.from_user
    ...
    if query.data == 'back_to_days':
        ...
        await query.message.reply_text(f"✅ Часовой пояс: **{context.user_data['timezone']}**\n\n📅 **Выберите день:**",
                                      parse_mode='Markdown', reply_markup=get_days_keyboard(offset))
        return DAY
...
async def admin_unblock_time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Разблокировка выбранного времени"""
    query = update.callback_query
    await query.answer()
...
def main():
    ...
    # ConversationHandler для записи пользователей
